	...
	go func() {
		...
		for {
			...
			default:
				...
				// If totalGenDur is 0 (no deltas yet), use elapsed time as fallback
				s.mu.Lock()
				...
				if tokensOut > 0 {
					var speed float64
					if totalGD.Seconds() > 0 {
						speed = float64(tokensOut) / totalGD.Seconds()
					} else if elapsed.Seconds() > 0 {
						// Fallback: use elapsed time (includes tool execution time, but better than nothing)
...
func (s *Spinner) UpdateTokens(tokensIn, tokensOut int, totalGenDur time.Duration) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.tokensIn = tokensIn
	s.tokensOut = tokensOut
	// Sync totalGenDur from local variable so speed is shown immediately
	// (spinner's totalGenDur may be stale if UpdateGenTime wasn't called recently)
	if totalGenDur > s.totalGenDur {
		s.totalGenDur = totalGenDur
	}
}
...
func FormatStatusLineEx(tokensIn, tokensOut int, dur, genDur time.Duration, ctxUsed, ctxMax int, providerName, modelName string, taskType ...string) string {
	...
	// Speed: use genDur (accumulated generation time)
	// If genDur is 0 (no deltas yet), use dur as fallback
	if tokensOut > 0 {
		var speed float64
		if genDur.Seconds() > 0 {
			speed = float64(tokensOut) / genDur.Seconds()
		} else if dur.Seconds() > 0 {
			// Fallback: use total duration (includes tool execution time, but better than nothing)
