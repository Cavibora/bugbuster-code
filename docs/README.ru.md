...
agent:
  ...
  # Rate limit (HTTP 429) — ретраи при лимите запросов
  rate_limit:
    max_retries: 50            # Макс. ретраев при 429 (default: 50)
    delay_ms: 3000             # Задержка между ретраями в мс (default: 3000)

  # Автопилот — автоматическое продолжение работы
  autopilot:
    max_iterations: 5000       # Макс. итераций автопилота (default: 5000)
    delay_ms: 2000             # Задержка между итерациями в мс (default: 2000)
...
| `reasoning.enabled` | bool | Включить reasoning/thinking токены (OpenRouter, OpenAI o-series) |
| `reasoning.effort` | string | Уровень reasoning: `max`, `xhigh`, `high`, `medium`, `low`, `minimal`, `none` |
| `reasoning.max_tokens` | int | Бюджет токенов для reasoning |
| `reasoning.exclude` | bool | Исключить reasoning из ответа (модель всё равно использует его внутри) |
...
| `agent.rate_limit.max_retries` | int | Макс. ретраев при HTTP 429 (rate limit). При исчерпании — показывается полная ошибка | 50 |
| `agent.rate_limit.delay_ms` | int | Задержка между ретраями при 429 (мс) | 3000 |
| `agent.autopilot.max_iterations` | int | Макс. итераций автопилота | 5000 |
| `agent.autopilot.delay_ms` | int | Задержка между итерациями автопилота (мс) | 2000 |
...
| `/auto` | Включить/выключить автопилот |
| `/auto N` | Включить автопилот с лимитом N итераций |
| `/rate` | Показать текущие настройки rate limit |
| `/rate <max_retries> [delay_ms]` | Настроить ретраи при 429 |

### Rate Limit (HTTP 429)

При HTTP 429 (лимит запросов) BugBuster автоматически ретраит с короткой задержкой вместо остановки. Полезно для бесплатных моделей (например `z-ai/glm-5.2:free` через OpenRouter), которые периодически лимитируют запросы.

**Поведение:**
- При 429 агент ретраит до `rate_limit.max_retries` раз (default 50) с задержкой `rate_limit.delay_ms` (default 3000мс).
- Сообщение о rate limit показывается пользователю, но **не сохраняется** в контекст.
- После исчерпания всех ретраев показывается полное сообщение об ошибке.
- **Автопилот не останавливается** на transient-ошибках (429, 404, 500, 502, 503, 504) — продолжает автоматически.

**Команды:**
| Команда | Описание |
|---------|----------|
| `/rate` | Показать текущие настройки |
| `/rate 100 2000` | Установить max_retries=100, delay=2000мс |
| `/rate 100` | Установить max_retries=100, задержку оставить |

### Автопилот

Автопилот автоматически продолжает работу агента после каждого ответа, пока план не завершён, не достигнут лимит итераций, или пользователь не прервал Ctrl+C.

**Поведение:**
- После каждого ответа агент проверяет, завершён ли план. Если нет — отправляет промпт продолжения.
- **Transient-ошибки (429, 404, 500, 502, 503, 504) не останавливают автопилот** — он продолжает после короткой задержки.
- Если стрим завершился ошибкой, автопилот **не** ложно сообщает о завершении плана — он ретраит.

**Команды:**
| Команда | Описание |
|---------|----------|
| `/auto` | Переключить автопилот (использует `max_iterations` из конфига) |
| `/auto N` | Включить автопилот с лимитом N итераций |
